    ...
    API.getSaves().then(function (data) {
        ...
        if (data.length > 0) {
            var $saves = data.map(function (artic) {
                ...
                var deleteButton = $("<button>").addClass("buttonMargin btn-small right red deleteIt").attr("type", "submit").attr("name", "action").text("Delete From Saved");
                var noteButton = $("<button>").addClass("buttonMargin btn-small modal-trigger right blue addNote").attr("type", "submit").attr("name", "action").text("Add Note").attr("data-target", "modal1");
